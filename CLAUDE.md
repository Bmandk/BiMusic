...
**`TrackTile` is a `ConsumerWidget`** — any test that renders `TrackTile` (directly or via a screen like `AlbumDetailScreen`) must have a `ProviderScope` ancestor. The widget only accesses `playlistProvider` inside the long-press context sheet (not during `build`), so tests that don't trigger long-press do not need `playlistServiceProvider` to be overridden; however, wrapping in `ProviderScope` is always required. Playlist integration tests use `nock` (call `nock.disableNetConnect()` + `nock.enableNetConnect('127.0.0.1')` in `beforeAll`, `nock.cleanAll()` in `afterEach`) since `GET /api/playlists/:id` now calls Lidarr.
