...
const stubArtist = {
  ...
  status: 'ended',
  ended: true,
  images: [{ url: '/image.jpg', coverType: 'poster', extension: '.jpg', remoteUrl: null }],
  path: '/music/artist',
  monitored: true,
  genres: ['Rock'],
...
describe('lidarrClient', () => {
  ...
  describe('getArtists', () => {
    it('returns artist list from GET /artist', async () => {
      nock(BASE).get('/api/v1/artist').matchHeader('X-Api-Key', API_KEY).reply(200, [stubArtist]);
  ...
  describe('getArtist', () => {
    it('returns single artist from GET /artist/:id', async () => {
      nock(BASE).get('/api/v1/artist/1').matchHeader('X-Api-Key', API_KEY).reply(200, stubArtist);
    ...
    it('throws 502 AppError when Lidarr returns 500', async () => {
      nock(BASE).get('/api/v1/artist/1').reply(500, { message: 'Internal Error' });
  ...
  describe('getAlbums', () => {
    ...
    it('sends artistId as query param when provided', async () => {
      nock(BASE).get('/api/v1/album').query({ artistId: '1' }).reply(200, [stubAlbum]);
  ...
  describe('lookupAlbum', () => {
    it('sends term as query param to GET /album/lookup', async () => {
      nock(BASE).get('/api/v1/album/lookup').query({ term: 'Abbey Road' }).reply(200, [stubAlbum]);
  ...
  describe('getTracks', () => {
    ...
    it('sends albumId query param when provided', async () => {
      nock(BASE).get('/api/v1/track').query({ albumId: '10' }).reply(200, [stubTrack]);
  ...
  describe('getTrackFiles', () => {
    it('sends albumId query param to GET /trackfile', async () => {
      nock(BASE).get('/api/v1/trackfile').query({ albumId: '10' }).reply(200, [stubTrackFile]);
  ...
  describe('search', () => {
    it('sends term as query param to GET /search', async () => {
      const stubResult = { id: 1, foreignId: 'abc', artist: stubArtist, album: stubAlbum };
      nock(BASE).get('/api/v1/search').query({ term: 'rock' }).reply(200, [stubResult]);
  ...
  describe('getAlbumCover', () => {
    it('streams response from GET /mediacover/album/:id/:filename', async () => {
      nock(BASE)
        ...
        .reply(200, Buffer.from('fake-cover-data'), { 'content-type': 'image/jpeg' });
