...
router.get('/:trackId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (bitrateRaw !== undefined) {
      const bitrateStr = typeof bitrateRaw === 'string' ? bitrateRaw : String(bitrateRaw);
      const parsed = parseInt(bitrateStr, 10);
      if (isNaN(parsed) || !VALID_BITRATES.has(parsed)) {
        res.status(400).json({
          error: { code: 'BAD_REQUEST', message: 'Invalid bitrate. Must be 128 or 320' },
        });
        return;
      }
      bitrate = parsed;
