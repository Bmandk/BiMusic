...
describe("GET /api/stream/:id/segment/:index — happy path", () => {
  ...
  it("deduplicates concurrent requests for the same segment", async () => {
    const filePath = path.join(fixtureDir, "track3.flac");
    const TRACK_ID = 3;
    const FILE_ID = 3;
    stubLidarr(TRACK_ID, FILE_ID, filePath, 2);

    mockState.delayMs = 50;
    mockState.callCount = 0;

    const [res1, res2] = await Promise.all([
      request(app)
        .get(`/api/stream/${TRACK_ID}/segment/001?bitrate=128`)
        .set("Authorization", `Bearer ${token}`),
      request(app)
        .get(`/api/stream/${TRACK_ID}/segment/001?bitrate=128`)
        .set("Authorization", `Bearer ${token}`),
...
describe("?token= query-param authentication", () => {
  ...
  it("segment: authenticates via ?token= without Authorization header", async () => {
    ...
    const res = await request(app).get(
      `/api/stream/${TRACK_ID}/segment/000?bitrate=128&token=${token}`,
    );
    // No Authorization header — only ?token=
  ...
  it("playlist: returns 401 when ?token= is absent and no Authorization header", async () => {
    const res = await request(app).get(
      `/api/stream/${TRACK_ID}/playlist?bitrate=128`,
    );
